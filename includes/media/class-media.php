...
class Media {
	...
	private function dependencies() {
		...
		include_once TIMS_PATH . 'includes/media/class-svg-support.php';

		// Replace WP gallery shortcode if Fancybox option is used.
		$fancybox = get_option( 'tims_enqueue_fancybox_script' );

		if ( $fancybox ) {
			require_once TIMS_PATH . 'includes/media/class-gallery-shortcode.php';
		}
	...
	public function image_sizes() {
		...
		add_image_size( __( 'meta-image', 'tims' ), 1200, 630, true );
		 ...
		 */
		add_image_size( __( 'column-thumbnail', 'tims' ), 48, 48, true );
	...
	public function rss_featured_images( $content ) {
		...
		$size = apply_filters( 'tims_rss_featured_image_size', 'medium' );
