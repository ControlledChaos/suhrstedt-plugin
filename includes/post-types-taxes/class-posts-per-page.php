 ...
 *
 * @since      1.0.0
 * @author     Greg Sweet <greg@timszine.com>
...
class Posts_Per_Page {
	...
	public function upgrade_check() {
		...
		if ( '1.3' === get_option( 'tims_ppp_upgrade', '1.3' ) ) {
			$this->activate();

			$tims_ppp_options = get_option( 'tims_ppp_options' );

			if ( isset( $tims_ppp_options['front_page_count'] ) ) {
				$tims_ppp_options['front_count'] = $tims_ppp_options['front_page_count'];
				unset( $tims_ppp_options['front_page_count'] );
			}

			if ( isset( $tims_ppp_options['index_count'] ) ) {
				$tims_ppp_options['front_count_paged'] = $tims_ppp_options['index_count'];
				unset( $tims_ppp_options['index_count'] );
			}

			update_option( 'tims_ppp_options', $tims_ppp_options );
			update_option( 'tims_ppp_upgrade', '1.4' );
		}
	...
	public function activate() {
		...
		$current_options   = get_option( 'tims_ppp_options' );
		$default_options   = [];
		...
		foreach ( $option_type_array as $option_type ) {
			...
			} elseif ( ! empty( $tims_ppp_options[ $option_type . '_count' ] ) ) {
		...
		update_option( 'tims_ppp_options', $default_options );
	...
	public function add_settings() {
		add_options_page( __( 'Posts Per Page', 'tims' ), __( 'Posts Per Page', 'tims' ), 'manage_options', 'posts-per-page', [ $this, 'view_settings' ] );
	}
		...
		<div class="wrap">
			...
			<form method="post" action="options.php">
				<?php
					...
					do_settings_sections( 'tims_ppp' );
					do_settings_sections( 'tims_ppp_custom' );
				?>
			<p class="submit"><input type="submit" class="button-primary" value="<?php _e( 'Save Changes', 'tims' ); ?>" /></p></form>
		</div>
		<?php
	}
	...
	public function register_settings() {
		...
		register_setting( 'tims_ppp_options', 'tims_ppp_options', [ $this, 'validate_options'] );

		// add_settings_section( 'tims_ppp_section_main', '', [ $this, 'output_main_section_text' ], 'tims_ppp' );
		add_settings_section( 'tims_ppp_section_custom', '', [ $this, 'output_custom_section_text' ], 'tims_ppp_custom' );

		add_settings_field( 'tims_ppp_index_count',     __( 'Main Index posts per page:', 'tims' ), [ $this, 'output_index_count_text' ],    'tims_ppp', 'tims_ppp_section_main' );
		add_settings_field( 'tims_ppp_category_count',  __( 'Category posts per page:', 'tims' ),   [ $this, 'output_category_count_text' ], 'tims_ppp', 'tims_ppp_section_main' );
		add_settings_field( 'tims_ppp_archive_count',   __( 'Archive posts per page:', 'tims' ),    [ $this, 'output_archive_count_text' ],  'tims_ppp', 'tims_ppp_section_main' );
		add_settings_field( 'tims_ppp_tag_count',       __( 'Tag posts per page:', 'tims' ),        [ $this, 'output_tag_count_text' ],      'tims_ppp', 'tims_ppp_section_main' );
		add_settings_field( 'tims_ppp_author_count',    __( 'Author posts per page:', 'tims' ),     [ $this, 'output_author_count_text' ],   'tims_ppp', 'tims_ppp_section_main' );
		add_settings_field( 'tims_ppp_search_count',    __( 'Search posts per page:', 'tims' ),     [ $this, 'output_search_count_text' ],   'tims_ppp', 'tims_ppp_section_main' );
		add_settings_field( 'tims_ppp_default_count',   __( 'Default posts per page:', 'tims' ),    [ $this, 'output_default_count_text' ],  'tims_ppp', 'tims_ppp_section_main' );

		add_settings_field( 'tims_ppp_post_type_count', '', [ $this, 'output_post_type_count_text' ], 'tims_ppp_custom', 'tims_ppp_section_custom' );
	...
	public function output_post_type_count_text() {
		...
		foreach ( $all_post_types as $p => $k ) {
			 ...
			 *  assign a good value. */
			if ( empty( $tims_ppp_options[ $p . '_count' ] ) ) {
				$tims_ppp_options[ $p . '_count' ] = 0;
			}

			if ( empty( $tims_ppp_options[ $p . '_count_paged' ] ) ) {
				$tims_ppp_options[ $p . '_count_paged' ] = 0;
			}
	...
	public function output_index_count_text() {
		...
		$tims_ppp_options = get_option( 'tims_ppp_options', [ 'front_count' => 0, 'front_count_paged' => 0 ] );

		?>
		<input id="tims_ppp_index_count[0]" name="tims_ppp_options[front_count]" size="10" type="text" value="<?php echo esc_attr( $tims_ppp_options[ 'front_count' ] ); ?>" />
		&nbsp;<input id="tims_ppp_index_count[1]" name="tims_ppp_options[front_count_paged]" size="10" type="text" value="<?php echo esc_attr( $tims_ppp_options[ 'front_count_paged' ] ); ?>" />
		<?php
	}
	...
	public function output_category_count_text() {
		...
		$tims_ppp_options = get_option( 'tims_ppp_options', [ 'category_count' => 0, 'category_count_paged' => 0 ] );

		?>
		<input id="cppppc_category_count[0]" name="tims_ppp_options[category_count]" size="10" type="text" value="<?php echo esc_attr( $tims_ppp_options[ 'category_count' ] ); ?>" />
		&nbsp;<input id="cppppc_category_count[1]" name="tims_ppp_options[category_count_paged]" size="10" type="text" value="<?php echo esc_attr( $tims_ppp_options[ 'category_count_paged' ] ); ?>" />
		<?php
	}
	...
	public function output_archive_count_text() {
		...
		$tims_ppp_options = get_option( 'tims_ppp_options', [ 'archive_count' => 0, 'archive_count_paged' => 0 ] );

		?>
		<input id="cppppc_archive_count[0]" name="tims_ppp_options[archive_count]" size="10" type="text" value="<?php echo esc_attr( $tims_ppp_options[ 'archive_count' ] ); ?>" />
		&nbsp;<input id="cppppc_archive_count[1]" name="tims_ppp_options[archive_count_paged]" size="10" type="text" value="<?php echo esc_attr( $tims_ppp_options[ 'archive_count_paged' ] ); ?>" />
		<?php
	}
	...
	public function output_tag_count_text() {
		...
		$tims_ppp_options = get_option( 'tims_ppp_options', [ 'tag_count' => 0, 'tag_count_paged' => 0 ] );

		?>
		<input id="tims_ppp_tag_count[0]" name="tims_ppp_options[tag_count]" size="10" type="text" value="<?php echo esc_attr( $tims_ppp_options[ 'tag_count' ] ); ?>" />
		&nbsp;<input id="tims_ppp_tag_count[1]" name="tims_ppp_options[tag_count_paged]" size="10" type="text" value="<?php echo esc_attr( $tims_ppp_options[ 'tag_count_paged' ] ); ?>" />
		<?php
	}
	...
	public function output_author_count_text() {
		...
		$tims_ppp_options = get_option( 'tims_ppp_options', [ 'author_count' => 0, 'author_count_paged' => 0 ] );

		?>
		<input id="tims_ppp_author_count[0]" name="tims_ppp_options[author_count]" size="10" type="text" value="<?php echo esc_attr( $tims_ppp_options[ 'author_count' ] ); ?>" />
		&nbsp;<input id="tims_ppp_author_count[1]" name="tims_ppp_options[author_count_paged]" size="10" type="text" value="<?php echo esc_attr( $tims_ppp_options[ 'author_count_paged' ] ); ?>" />
		<?php
	}
	...
	public function output_search_count_text() {
		...
		$tims_ppp_options = get_option( 'tims_ppp_options', [ 'search_count' => 0, 'search_count_paged' => 0 ] );

		?>
		<input id="cppppc_search_count[0]" name="tims_ppp_options[search_count]" size="10" type="text" value="<?php echo esc_attr( $tims_ppp_options[ 'search_count' ] ); ?>" />
		&nbsp;<input id="cppppc_search_count[1]" name="tims_ppp_options[search_count_paged]" size="10" type="text" value="<?php echo esc_attr( $tims_ppp_options[ 'search_count_paged' ] ); ?>" />
		<?php
	}
	...
	public function output_default_count_text() {
		...
		$tims_ppp_options = get_option( 'tims_ppp_options', [ 'default_count' => 0, 'default_count_paged' => 0 ] );

		?>
		<input id="cppppc_default_count[0]" name="tims_ppp_options[default_count]" size="10" type="text" value="<?php echo esc_attr( $tims_ppp_options[ 'default_count' ] ); ?>" />
		&nbsp;<input id="cppppc_default_count[1]" name="tims_ppp_options[default_count_paged]" size="10" type="text" value="<?php echo esc_attr( $tims_ppp_options[ 'default_count_paged' ] ); ?>" />
		<?php
	}
	...
	public function modify_query( $query ) {
		...
		if ( $query->is_home() ) {
			$this->process_options( 'front', $tims_ppp_options );
		} elseif ( $query->is_post_type_archive( $post_type_array ) ) {
			$current_post_type_object = $query->get_queried_object();
			$this->process_options( $current_post_type_object->name, $tims_ppp_options );
		} elseif ( $query->is_category() ) {
			$this->process_options( 'category', $tims_ppp_options );
		} elseif ( $query->is_tag() ) {
			$this->process_options( 'tag', $tims_ppp_options );
		} elseif ( $query->is_author() ) {
			$this->process_options( 'author', $tims_ppp_options );
		} elseif ( $query->is_search() ) {
			$this->process_options( 'search', $tims_ppp_options );
			...
			$this->process_options( 'archive', $tims_ppp_options );
		} else {
			$this->process_options( 'default', $tims_ppp_options );
		}
	 ...
	 * @param  $tims_ppp_options array of options from the database for custom posts per page.
	 * @return void
	 */
	public function process_options( $option_prefix, $tims_ppp_options ) {

		if ( ! $this->paged_view && ! empty( $tims_ppp_options[ $option_prefix . '_count' ] ) ) {

			$this->final_options['posts']  = $tims_ppp_options[ $option_prefix . '_count' ];
			$this->final_options['offset'] = 0;
			$this->final_options['set_count'] = $tims_ppp_options[ $option_prefix . '_count' ];
			$this->final_options['set_count_paged'] = $tims_ppp_options[ $option_prefix . '_count_paged' ];

		} elseif ( $this->paged_view & ! empty( $tims_ppp_options[ $option_prefix . '_count_paged' ] ) ) {

			$this->page_count_offset = ( $tims_ppp_options[ $option_prefix . '_count_paged' ] - $tims_ppp_options[ $option_prefix . '_count' ] );
			$this->final_options['offset']  = ( ( $this->page_number - 2 ) * $tims_ppp_options[ $option_prefix . '_count_paged' ] + $tims_ppp_options[ $option_prefix . '_count' ] );
			$this->final_options['posts']   = $tims_ppp_options[ $option_prefix . '_count_paged' ];
			$this->final_options['set_count'] = $tims_ppp_options[ $option_prefix . '_count' ];
			$this->final_options['set_count_paged'] = $tims_ppp_options[ $option_prefix . '_count_paged' ];
