...
class Meta_Image {
	...
	public function __construct() {
		...
		add_action( 'tims_meta_image_tag', [ $this, 'image' ] );
	...
	public function image() {
		...
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			...
			$blog_image       = get_field( 'tims_meta_blog_image', 'option' );
			$default_image    = get_field( 'tims_meta_default_image', 'option' );
			$feature_image    = get_field( 'tims_project_image' );
			$feature_vimeo    = get_field( 'tims_project_vimeo_id' );
			$commercial_image = get_field( 'tims_commercial_thumbnail' );
			$commercial_vimeo = get_field( 'tims_commercial_vimeo_id' );
			...
			} elseif ( is_singular( 'tims_features' ) && ! empty( $feature_image ) ) {
				$size = 'meta-image';
				$src  = $feature_image['sizes'][$size];

			// If on a single Commercial page and a thumbnail image has been selected.
			} elseif ( is_singular( 'tims_commercials' ) && ! empty( $commercial_image ) ) {
				$size = 'meta-image';
