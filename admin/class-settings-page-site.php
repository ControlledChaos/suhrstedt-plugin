...
class Settings_Page_Site {
    ...
    public function settings_page() {
		...
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			...
			$position   = get_field( 'tims_settings_link_position', 'option' );

			// The label of the page in the admin menu.
			$link_label = get_field( 'tims_site_settings_link_label', 'option' );

			// The icon used for the pagin in the admin menu.
			$link_icon  = get_field( 'tims_site_settings_link_icon', 'option' );
		...
		// If the Advanced Custom Fields Pro plugin is not active.
			...
			// The label of the page in the admin menu.
			$link_label = sanitize_text_field( get_option( 'tims_site_settings_link_label' ) );

			// The icon used for the pagin in the admin menu.
			$link_icon  = sanitize_text_field( get_option( 'tims_site_settings_link_icon' ) );
			...
			// Otherwise use the Admin Settings icon.
				...
				$icon = __( 'dashicons-admin-settings', 'tims' );
			}
			...
			if ( isset( $_GET['page'] ) ) {
				...
				if (
					...
					&& ( $_GET['page'] == TIMS_ADMIN_SLUG . '-settings' )
				) {
					wp_redirect( admin_url( 'admin.php?page=' . TIMS_ADMIN_SLUG . '-settings&tab=admin-menu' ), 302 );
				...
				// If on the top level page and top level option is not selected.
					...
					wp_redirect( admin_url( 'index.php?page=' . TIMS_ADMIN_SLUG . '-settings&tab=admin-menu' ), 302 );
				}
			}
	...
	public function help_acf_settings_notice() {
		...
		include_once TIMS_PATH . 'admin/partials/help/help-acf-notice.php';
    ...
    public function page_help_section_sidebar() {
		...
		$html = '<ul>
			<li><a href="https://www.advancedcustomfields.com/resources/" target="_blank" style="text-decoration: none;">' . __( 'ACF Documentation', 'tims' ) . '</a></li>
		</ul>';
    ...
    public function page_output() {
		...
		require TIMS_PATH . 'admin/partials/settings-page-site.php';
	...
	public function acf_docs_link() {
		...
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			...
			$submenu['edit.php?post_type=acf-field-group'][] = [ __( 'Documentation', 'tims' ), 'manage_options', $url ];
 ...
 */
function tims_settings_page_site() {
