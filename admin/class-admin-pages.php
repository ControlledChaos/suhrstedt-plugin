...
class Admin_Pages {
    ...
    public function __construct() {
        ...
        if ( tims_acf_options() ) {
            ...
            $admin_header = get_field( 'tims_use_admin_header', 'option' );
        } else {

            // Otherwise look for the WP API setting.
            $admin_header = get_option( 'tims_use_admin_header' );
        }
    ...
    public function about_plugin() {
        ...
        if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
            ...
            // Get the field.
            $link_label = get_field( 'tims_site_plugin_link_label', 'option' );

        // If ACF is not active, get the field from the WordPress options page.
        } else {

            // Get the field.
            $link_label = sanitize_text_field( get_option( 'tims_site_plugin_link_label' ) );
        }
        ...
        if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
            ...
            // Get the field.
            $link_icon  = get_field( 'tims_site_plugin_link_icon', 'option' );

        // If ACF is not active, get the field from the WordPress options page.
        } else {

            // Get the field.
            $link_icon  = sanitize_text_field( get_option( 'tims_site_plugin_link_icon' ) );
        }
        ...
        // Otherwise use the scholar's cap icon to imply instruction.
            ...
            $icon = __( 'dashicons-welcome-learn-more', 'tims' );
        }
    ...
    public function about_plugin_output() {
        ...
        require TIMS_PATH . 'admin/partials/plugin-page-about.php';
    ...
    public function help_about_plugin() {
		...
		$screen->add_help_tab( [
			'id'       => 'help_plugin_info',
			'title'    => __( 'More Information', 'tims' ),
			'content'  => null,
	...
	public function help_plugin_info() {
		...
		include_once TIMS_PATH . 'admin/partials/help/help-plugin-info.php';
	...
	public function help_convert_plugin() {
		...
		include_once TIMS_PATH . 'admin/partials/help/help-plugin-convert.php';
    ...
    public function help_about_page_sidebar() {
        ...
        $html  = sprintf( '<h4>%1s</h4>', __( 'Author Credits', 'tims' ) );
        $html .= sprintf(
            '<p>%1s %2s.</p>',
            __( 'This plugin was originally written by', 'tims' ),
            'Greg Sweet'
        );
        $html .= sprintf(
            ...
            __( 'for more free downloads.', 'tims' )
        );
        $html .= sprintf(
            '<p>%1s</p>',
            __( 'Change this sidebar to give yourself credit for the hard work you did customizing this plugin.', 'tims' )
         );
	...
	public function admin_header() {
		...
		if ( ! empty( $admin_header ) ) {
			...
			include_once TIMS_PATH . 'admin/partials/admin-header.php';
		}
    ...
    public function title_placeholders( $title ) {
        ...
        if ( 'post' == $screen->post_type ) {
            $post_title = esc_html__( 'Post Title', 'tims' );

        // Post type: page.
        } elseif ( 'page' == $screen->post_type ) {
            $post_title = esc_html__( 'Page Title', 'tims' );
        ...
        // Post type: custom, unidentified.
            ...
            $post_title = esc_html__( 'Enter Title', 'tims' );
        }

        // Apply a filter conditional modification.
        $title = apply_filters( 'tims_post_title_placeholders', $post_title );
    ...
    private function get_column_image( $post_ID ) {
        ...
        $thumb = apply_filters( 'tims_column_thumbnail_size', $size );
    ...
    public function image_column_head( $defaults ) {
        ...
        $name    = __( 'Featured Image', 'tims' );

        // Apply a filter for conditional modification.
        $heading = apply_filters( 'tims_image_column_head', $name );
    ...
    ?>
        ...
        <?php echo sprintf( '<p>%1s</p>', __( 'Add a description for internal use and for search engines.', 'tims' ) );
    ...
    function remove_screen_options( $display_boolean, $wp_screen_object ) {
        ...
        } elseif ( in_array( $GLOBALS['pagenow'], [ 'options-general.php' ] ) && ( ( $_GET['page'] == 'tims-scripts' ) || ( $_GET['page'] == 'sort-order-settings' ) || ( $_GET['page'] == 'posts-per-page' ) ) ) {
