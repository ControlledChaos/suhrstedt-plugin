...
class Welcome {
    ...
    public function __construct() {
		...
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			$dismiss = get_field( 'tims_remove_welcome_dismiss', 'option' );

		// If ACF is not active, get the field from the WordPress options page.
		} else {
			$dismiss = get_option( 'tims_remove_welcome_dismiss' );
		}
		...
		// If ACF is active, get the field from the ACF options page.
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			$welcome = get_field( 'tims_custom_welcome', 'option' );
		} else {
	...
	public function widget_areas() {
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - First Area', 'tims' ),
			'id'            => 'tims_welcome_widget_first',
			'description'   => __( '', 'tims' ),
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - Second Area', 'tims' ),
			'id'            => 'tims_welcome_widget_second',
			'description'   => __( '', 'tims' ),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h3>',
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - Third Area', 'tims' ),
			'id'            => 'tims_welcome_widget_last',
			'description'   => __( '', 'tims' ),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h3>',
	...
	public function styles() {
        ...
        if ( $screen->id == 'dashboard' ) {
            wp_enqueue_style( TIMS_ADMIN_SLUG . '-welcome', TIMS_URL .  'admin/dashboard/assets/css/welcome.css', [], null, 'screen' );
        }
